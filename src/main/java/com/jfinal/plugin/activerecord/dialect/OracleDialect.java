...
public class OracleDialect extends Dialect {
	...
	// insert into table (id,name) values(seq.nextval, 锛�
	@Override
		...
		for (Entry<String, Object> e: attrs.entrySet()) {
			...
			if (table.hasColumnLabel(colName)) {
				...
				if (value instanceof String && isPrimaryKey(colName, pKeys) && ((String)value).endsWith(".nextval")) {
					...
					temp.append("to_date('"+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(value) +"', 'yyyy-mm-dd HH24:MI:SS')");
				} else {
				    temp.append("?");
				    paras.add(value);
				}
		...
		for (Entry<String, Object> e : attrs.entrySet()) {
			...
			if (modifyFlag.contains(colName) && table.hasColumnLabel(colName)) {
				...
				if (e.getValue() instanceof java.sql.Date || e.getValue() instanceof java.sql.Timestamp) {
					sql.append("to_date('"+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(e.getValue()) +"', 'yyyy-mm-dd HH24:MI:SS')");
				} else {
					sql.append(" = ? ");

...
public class Button extends BaseModel<Button> {
	...
	public List<Button> queryByMenuCode(String menuCode, int rid) {
		...
		return Button.dao.find(
				"select * from eova_button where menu_code = ? and ui is not null and id in (select bid from eova_role_btn where rid = ?) order by order_num",
				menuCode, rid);
	}
	...
	public boolean isExistButton(String menuCode, String bs) {
		String sql = "select count(*) from eova_button where menu_code = ? and bs = ?";
	...
	public void deleteFunByMenuCode(String menuCode) {
		String sql = "delete from eova_button where name != ? and menu_code = ?";

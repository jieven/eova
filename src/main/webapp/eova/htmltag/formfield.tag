<%if(item.type == "下拉框"){%>
	<div><#combo id="${item.en}" name="${item.en}" code="${item.object_code}" field="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" multiple="${item.is_multiple}" /></div>
...
<%} else if(item.type == "文本域"){%>
	<div><#texts id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" placeholder="${item.placeholder!}" validator="${item.validator!}" style="width:758px;height:${item.height!20}px;" /></div>
...
<%} else {// 默认为文本框%>
    <div><#text id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" placeholder="${item.placeholder!}" isNoN="${item.is_required!}" validator="${item.validator!}" options="" /></div>
<%}%>
...
<%} else if(item.type == "文本域"){%>
	<div><#texts id="${item.en}" name="${item.en}" value="${item.value!item.valueExp}" isNoN="${item.isNotNull!}" placeholder="${item.placeholder!}" validate="${item.validator!}" style="width:758px;height:30px;" /></div>
...
<%} else {// 默认为文本框%>
    <div><#text id="${item.en}" name="${item.en}" value="${item.value!item.valueExp}" placeholder="${item.placeholder!}" isNoN="${item.isNotNull!}" validate="${item.validator!}" options="" /></div>
<%}%>
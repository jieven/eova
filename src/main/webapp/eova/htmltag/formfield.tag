...
<%} else if(item.type == "查找框"){%>
    <div><#find id="${item.en}" name="${item.en}" code="${item.object_code}" field="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" disable="${item.is_disable!}" /></div>
<%} else if(item.type == "时间框"){%>
    <div><#time id="${item.en}" name="${item.en}" value="${strutil.formatDate(item.value!date(), 'yyyy-MM-dd HH:mm:ss')}" isNoN="${item.is_required!}" disable="${item.is_disable!}" /></div>
...
<%} else if(item.type == "图片框"){%>
	<div><#img id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" disable="${item.is_disable!}" /></div>
<%} else if(item.type == "文件框"){%>
	<div><#file id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" disable="${item.is_disable!}" /></div>

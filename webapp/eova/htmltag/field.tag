...
<%} else if(item.type == "时间框"){%>
    <div><#time id="${item.en}" name="${item.en}" value="${isEmpty(item.value!) ? '' : strutil.formatDate(item.value!date(), 'yyyy-MM-dd HH:mm:ss')}" isNoN="${item.is_required!}" disable="${item.is_disable!}" options="format:'yyyy-MM-dd HH:mm:ss'" /></div>
<%} else if(item.type == "日期框"){%>
    <div><#time id="${item.en}" name="${item.en}" value="${isEmpty(item.value!) ? '' : strutil.formatDate(item.value!date(), 'yyyy-MM-dd')}" isNoN="${item.is_required!}" disable="${item.is_disable!}" options="format:'yyyy-MM-dd'"/></div>
...
<%} else if(item.type == "地图框"){%>
    <div><#map id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" disable="${item.is_disable!}" /></div>
<%} else {// 默认为文本框%>

...
//确认上架、下架
var gameWork = function(grid, title, url){
	$.messager.confirm('请确认', '当前所选游戏' + title + '吗？', function(o) {
		if (o) {
			$.ajax({
				url : url,
				dataType : 'json',
...
//确认公会显示、隐藏
var gmsWork = function(grid, title, url){
	$.messager.confirm('请确认', '当前所选公会' + title + '吗？', function(o) {
		if (o) {
			$.ajax({
				url : url,
				dataType : 'json',
...
var exportFun = function() {
	$.messager.alert('提示', '导出功能正在开发中！', 'info');
};

var importFun = function() {
	$.messager.alert('提示', '导入功能正在开发中！', 'info');
};
const {createApp, ref, reactive, watch, onMounted, nextTick, useTemplateRef, defineAsyncComponent} = Vue; // 使用全局 Vue 对象
const {me} = EovaUI
const {x} = EovaTools
...
const app = createApp({
    setup() {
        ...
        const onSubmit = (id) => {
            ...
            let data_ = refForm.value.getData()

            // 更新
            let url = me.urls.url('form_add', props)
        ...
        // 按需获取当前模版的refs
        // let refs = {}
        // let refKey = uzoo.vue.useTemplateRef();
        // refKey.forEach(key => {
        //     refs[key] = useTemplateRef(key);
        // })
